...
",").split(f);l=0;for(m=d.length;l<m;l++){o=O(d[l]);c=j.properties;y=0;for(v=c.length;y<v;y++){e=c[y];b=O(e.property);e=O(e.value);if(b){b=b.replace("-moz-box-",E);switch(b){case "display":e==="-moz-box"&&h(o,j,x,x);break;case "orient":case "align":case "direction":case "pack":h(o,j,x,x);break;case "flex":case "flex-group":case "ordinal-group":h(o,j,b,e)}}}}}for(k in n)n.hasOwnProperty(k)&&B.push(n[k]);for(k in i)i.hasOwnProperty(k)&&la.push(i[k]);return{boxes:B,children:la}}function ma(a,d,c){var b,
e,f=[],g,h,k,i,n,r,p;g=0;for(h=c.length;g<h;g++){k=c[g];if(k.selector){b=d(k.selector);b=b[0]?b:[b];if(b[0]){i=0;for(n=b.length;i<n;i++){r=b[i];if(r.nodeName!==na)switch(r.nodeName.toLowerCase()){case "script":case "style":case "link":break;default:if(r.parentNode===a){ca(r);e={};for(p in k)if(k.hasOwnProperty(p))e[p]=k[p];e.match=r;f.push(e)}}}}}else{ca(k);f.push({match:k,selector:ja(k)})}}return f}function oa(a,d,c){c=c.replace(c.charAt(0),c.charAt(0).toUpperCase());c=a["offset"+c]||0;var b,e,f;
if(c){b=0;for(e=d.length;b<e;b++){f=parseFloat(a.currentStyle[d[b]]);isNaN(f)||(c-=f)}}return c}function pa(a,d){var c,b,e=a.currentStyle&&a.currentStyle[d],f=a.style;if(!qa.test(e)&&Ka.test(e)){c=f.left;b=a.runtimeStyle.left;a.runtimeStyle.left=a.currentStyle.left;f.left=e||0;e=f.pixelLeft+"px";f.left=c||0;a.runtimeStyle.left=b}return e}function D(a,d,c){if(a!==na){if(H.getComputedStyle)a=H.getComputedStyle(a,x)[d];else if(La.test(d)){var b=a&&a.currentStyle?a.currentStyle[d]:0;if(!qa.test(b)){if(b===
"auto"||b==="medium"){switch(d){case "width":b=[ra,sa,ta,ua];b=oa(a,b,d);break;case "height":b=[ea,va,wa,xa];b=oa(a,b,d);break;default:b=pa(a,d)}a=b}else a=pa(a,d);b=a}a=b}else a=a.currentStyle[d];if(c){a=parseInt(a,10);if(isNaN(a))a=0}return a}}function Ma(a){return a.innerWidth||a.clientWidth}function Na(a){return a.innerHeight||a.clientHeight}function fa(a,d,c,b){var e=[],f,g,h;f=0;for(g=ya.length;f<g;f++){h=ya[f];e.push((b?h:E)+d+":"+(!b?h:E)+c)}a.style.cssText+=e.join(";");return a}function M(a,
d,c){var b=a&&a[0]?a:[a],e,f;e=0;for(f=b.length;e<f;e++)if((a=b[e])&&a.style)a.style[d]=c?c+"px":E}function Oa(a){var d,c,b,e,f;a=a.replace(Pa,function(g,h){return"%"+h}).replace(/\s|\>|\+|\~/g,"%").split(/%/g);d={_id:100,_class:10,_tag:1};b=c=0;for(e=a.length;b<e;b++){f=a[b];if(/#/.test(f))c+=d._id;else if(/\.|\[|\:/.test(f))c+=d._class;else if(/[a-zA-Z]+/.test(f))c+=d._tag}return c}function Qa(a,d,c){d=[];var b,e=(c?"ordinal":"flex")+"Specificity",f,g,h,k,i,n;f=0;for(g=a.length;f<g;f++){h=a[f];
if(!c&&h.flex||c&&h["ordinal-group"]){h[e]=h[e]||Oa(h.selector);b=U;k=0;for(i=d.length;k<i;k++){n=d[k];if(n.match===h.match){if(n[e]<h[e])d[g]=h;return U}}b||d.push(h)}}return d}function Z(a,d,c){var b={},e=[],f=0,g,h,k,i,n,r,p,j;a=Qa(a,d,c);h=0;for(k=d.length;h<k;h++){i=d[h];n=0;for(r=a.length;n<r;n++){p=a[n];if(c){g=p["ordinal-group"]||"1";if(p.match===i){p.match.setAttribute("data-ordinal-group",g);b[g]=b[g]||[];b[g].push(p)}}else{g=p.flex||"0";if(p.match===i&&(!p[g]||p[g]&&parseInt(p[g],10)<=
1)){f+=parseInt(g,10);b[g]=b[g]||[];b[g].push(p)}}}if(c&&!i.getAttribute("data-ordinal-group")){g="1";i.setAttribute("data-ordinal-group",g);b[g]=b[g]||[];b[g].push({match:i})}}for(j in b)b.hasOwnProperty(j)&&e.push(j);e.sort(function(l,m){return m-l});return{keys:e,groups:b,total:f}}function Ra(){if(!za){var a,d,c,b,e=R.body,f=R.documentElement,g;ia("resize",function(){g&&window.clearTimeout(g);g=window.setTimeout(function(){c=H.innerWidth||f.innerWidth||f.clientWidth||e.clientWidth;b=H.innerHeight||
...
this.setup(c.target,c.nodes,c)}b=b.parentNode}}};J.updateInstance=function(a,d){var c,b;if(a)if((c=B[a.FLX_DOM_ID])&&c._instance)c._instance.updateModel(c);else c||new J.box(d);else for(b in B)if(B.hasOwnProperty(b))(c=B[b])&&c._instance&&c._instance.updateModel(c)};J.getInstance=function(a){return B[a.FLX_DOM_ID]};J.destroyInstance=function(a){var d,c,b,e,f;d=function(g){g.target.FLX_DOM_ID=x;g.target.style.cssText=E;c=0;for(b=g.children.length;c<b;c++){e=g.children[c];e.match.style.cssText=E}};
if(a)(a=B[a.FLX_DOM_ID])&&d(a);else{for(f in B)B.hasOwnProperty(f)&&d(B[f]);B=[]}};J.flexboxSupport=function(){var a={},d,c=R.createElement("flxbox"),b,e,f;c.style.width=c.style.height="100px";c.innerHTML='<b style="margin: 0; padding: 0; display:block; width: 10px; height:50px"></b><b style="margin: 0; padding: 0; display:block; width: 10px; height:50px"></b><b style="margin: 0; padding: 0; display:block; width: 10px; height:50px"></b>';fa(c,"display","-moz-box",x);fa(c,"-moz-box-align","stretch",
u);fa(c,"-moz-box-pack","justify",u);R.body.appendChild(c);d=c.firstChild.offsetHeight;b={boxAlignStretch:function(){return d===100},boxPackJustify:function(){var g=0,h,k;h=0;for(k=c.childNodes.length;h<k;h++)g+=c.childNodes[h].offsetLeft;return g===135}};for(f in b)if(b.hasOwnProperty(f)){e=b[f];e=e();if(!e)a.partialSupport=u;a[f]=e}R.body.removeChild(c);return~c.style.display.indexOf("-moz-box")?a:U};J.init=function(){J.flexboxSupported=Q=J.flexboxSupport();if((!Q||Q.partialSupport)&&T)Ea()};J.version=
"1.0.3";(function(a){if(!Ca){var d,c,b;for(c in V)if(V.hasOwnProperty(c)){b=V[c];if(H[c]&&!d)if(d=eval(b.s.replace("*",c))){Ca=c;break}}T=d}var e,f;for(f in V)if(V.hasOwnProperty(f)){b=V[f];if(H[f]&&!e&&b.m){e=eval(b.m.replace("*",f));d=b.c?eval(b.c.replace("*",f)):H;c=[];if(e&&d){b.p&&c.push(b.p);c.push(a);e.apply(d,c);break}}}e||ia("load",a)})(J.init);return J}(this,document);
